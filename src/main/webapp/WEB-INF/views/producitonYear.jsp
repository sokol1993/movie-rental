...
<body>
	...
	<div class="container-fluid">
		<div class="collapse navbar-collapse" id="myNavbar">
			<ul class="nav navbar-nav">
				...
				<li class="active"><a href="years.html"><spring:message code="label.addYear" /></a></li>
			</ul>
	...
	<div class="container">
		<h3><spring:message code="label.year"/></h3>

		<form:form role="post" method="post" action="addYear.html"
			commandName="producitonYear">

			<tr>
				<td><form:hidden path="id" />
			</tr>
			<div class="form-group">
			<spring:message code="label.enterYear" var="enter1"/>
				...
				<form:input path="year" class="form-control" id="year"
					...
					<span style='color: red'><form:errors path="year"/></span>
			</div>
			<c:if test="${producitonYear.id==0}">
				<button type="submit" class="btn btn-default"><spring:message code="label.addYear"/></button>
			</c:if>
			<c:if test="${producitonYear.id!=0}">
				<button type="submit" class="btn btn-default"><spring:message code="label.editYear"/></button>
			</c:if>
		</form:form>
	</div>
	<div class="container">
		<h3><spring:message code="label.yearList"/></h3>
		<c:if test="${!empty producitonYearList}">
			<table class="table">
				<tr>
					<th><spring:message code="label.year"/></th>
					<th>&nbsp;</th>
				</tr>
				<c:forEach items="${producitonYearList}" var="year">
					<tr>
						<td>${year.year}</td>
						<td><a href="years.html?yearId=${year.id}"><spring:message code="label.edit"/></a></td>
					</tr>
				</c:forEach>
			</table>
		</c:if>
