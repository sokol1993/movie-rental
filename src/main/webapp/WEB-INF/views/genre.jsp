...
<c:url value="/j_spring_security_logout" var="logoutUrl" />
	...
	<div class="container-fluid">
		<div class="collapse navbar-collapse" id="myNavbar">
			<ul class="nav navbar-nav">
				...
				<li class="active"><a href="genres.html"><spring:message code="label.addGenre" /></a></li>
	...
	<div class="container">
	<h3><spring:message code="label.genre" /></h3>


		<form:form role="form" method="post" action="addGenre.html"
			commandName="genre">
			...
			<spring:message code="label.enterGenre" var = "enter1"/>
				<label for="genreName"><spring:message code="label.genre" /></label>
				<form:input path="genreName" class="form-control" id="genreName"
					placeholder="${enter1 }"></form:input>
					<span style='color: red'><form:errors path="genreName"/></span>
			</div>
			<c:if test="${genre.id==0}">
				<button type="submit" class="btn btn-default"><spring:message code="label.addGenre" /></button>
			</c:if>
			<c:if test="${genre.id!=0}">
				<button type="submit" class="btn btn-default"><spring:message code="label.editGenre" /></button>
			</c:if>
		</form:form>
	</div>
	<div class="container">
		...
		<table class="table">
			...
			<c:forEach items="${genreList}" var="genre">
				<tr>
					...
					<td><a href="genres.html?genreId=${genre.id}"><spring:message code="label.edit" /></a></td>
				</tr>
			</c:forEach>
		</table>
	</c:if>
