...
<body>
	...
	<div class="container-fluid">
		<div class="collapse navbar-collapse" id="myNavbar">
			<ul class="nav navbar-nav">
				<li><a href="users.html"><spring:message code="label.register"/></a></li>
				<li class="active"><a href="rents.html"><spring:message code="label.rentMovie"/></a></li>
			</ul>
			...
			<table class="table">
				...
				<c:forEach items="${userList}" var="userID">
					<c:forEach items="${movieList}" var="movie" varStatus="index">
						<form:form method="post" action="addRent.html" commandName="rent">
							<tr>
								<td>${movie.title}</td>
								...
								<c:if test="${movie.available=='NIE'}">
									<td></td>
									<td></td>
									<td></td>
								</c:if>
								<c:if test="${movie.available=='TAK'}">
								<spring:message code="label.enterDate" var="enter1"/>
									<td><input type="text" name="returnDate"
										class="datepicker" id="${index.index}" placeholder="${enter1 }" /></td>
										<td><span style='color: red'><form:errors path="returnDate"/></span></td>
									<input type="hidden" name="idMovie" value="${movie.id}" />
									<input type="hidden" name="idUser" value="${userID.id}" />
									<td><input type="submit" class="btn btn-default"
										value="<spring:message code="label.rent"/>" /></td>
								</c:if>
							</tr>
						</form:form>
					</c:forEach>
	...
	<div class="container">
		<h3><spring:message code="label.myrents"/></h3>
		<table class="table">
			<tr>
				<th><spring:message code="label.title"/></th>
				<th><spring:message code="label.returnDate"/></th>
